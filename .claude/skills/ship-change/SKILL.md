...
name: ship-change
description: >
  Ship a completed change end-to-end in the commandeer repo: commit + push, rebuild
  the release binary, then restart the running app. Invoke this AFTER every finished
...
git remote and the running process both reflect the finished work. Do all three steps
...
- Use `pkill -x` (never `pkill -f`) on Linux/macOS; `Stop-Process -Name` on Windows.
- The single-instance plugin means launching while an instance is alive just **toggles**
  the palette — so kill first, then launch, to get a clean restart on new code.
...
- **Windows:** if the app runs from the tray, quit it there (or `Stop-Process`) before
  launching the freshly built exe.
