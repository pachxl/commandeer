...
export default function ConfirmOverlay({ options, remember, focus, onToggleRemember, onResolve }: ConfirmOverlayProps) {
  const confirmBg = focus === 'confirm' ? (options.danger ? '#f7768e' : 'var(--accent)') : 'transparent'
    ...
    >
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          {options.detail && (
            <span style={{ fontSize: 12, fontFamily: 'var(--font)', color: 'var(--text-dim)' }}>{options.detail}</span>
          )}
        </div>
        ...
        {options.key && (
          ...
          >
            ...
            <span style={{ fontSize: 12, fontFamily: 'var(--font)', color: 'var(--text-dim)' }}>Don't ask again</span>
          </div>
        )}
